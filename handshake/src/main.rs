...
use std::{fmt::Display, io::Error, net::{IpAddr, Ipv4Addr, Ipv6Addr, SocketAddr, TcpStream}, str::FromStr};
...
fn get_socket(ip_address: &str, port: &str) -> Result<SocketAddr, HandshakeError> {
    ...
    .map_err(|_| HandshakeError::BadTargetPortInteger)?;
...
fn main() {
    ...
    match result {
        Ok(result) => println!("Handshake with {}:{} was successful: {}", target_node, target_port, result),
